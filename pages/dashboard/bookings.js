...
const Index = () => {
  ...
  const getbooking = async (id, page) => {
    ...
    const response = await Api(
      "get",
      `api/expert/booking/${id}?type=${values}&page=${page}&limit=10`
    );
  ...
  const statusBooking = async (bookingID, productID) => {
    const response = await Api(
      "put",
      `api/customer/booking/rating/${bookingID}?product_id=${productID}`
    );
    if (response.status == 200) {
  ...
  const cancelBookingHandler = async (bookingID, productID) => {
    const response = await Api(
      "put",
      `api/expert/booking/${UserID}?booking_id=${bookingID}&product_id=${productID}`
    );
    if (response.status == 200) {
      toast.success(response?.data?.msg);
      getbooking(UserID, currentPage);
          ...
          <Box className={`${styles.tabZIndexTop}`} sx={{ width: "100%" }}>
            ...
            >
              ...
              <Tab
                ...
                className={values === "past" ? "activeLink" : "tab_1_style"}
                value={"past"}
              />
            </Tabs>
          </Box>
                          ...
                          <div className="flex-calenderBox-Btns">
                            <div className="d-flex pt-1">
                              <div className="calenderBookingBox mt-1">
                                ...
                                <div className="monthNameCalender">
                                  {moment(value?.date).format("MMM ")}
                                </div>
                              </div>
                              <div className="mt-1">
                                <div className="BookingMassage-NameExpert">
                                  {value?.service_id?.name}
                                </div>
                                <div className="BookingMassage-Time pt-1">
                                  {value?.duration} min
                                </div>
