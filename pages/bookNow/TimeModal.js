        ...
        if (pkg?.option_id === option?._id) {
          if (option.mandatory) {
            if (selectedPackage.length < 1) {
              const isPackageSelected = selectedPackage?.find(
                (e) => e?.mandatory && option?._id === e?.option_id
              );
              if (!isPackageSelected) {
                ...
                pkg.mandatory = true;
                setServiceObject({ ...serviceObject });
                const filter = selectedPackage?.filter(
                  (e) => e?.option_id !== option?._id
                );
                filter.push(pkg);
  ...
  return (
    <>
      ...
      <Modal
        ...
        size="lg"
        centered
        className={`${styles.TimeModalHeight}`}
      >
        <Modal.Header
          closeButton
          className={`${styles.ModalHead} ${styles.ShadowClass} ${styles.TimeModalHeadClass}`}
        >
          <Modal.Title>
                ...
                <div key={index}>
                  ...
                  {serviceObject?.packages?.map((pkg, i) =>
                    pkg?.option_id === option?._id ? (
                      <div
                        ...
                        onClick={() => {
                          if (option.mandatory) {
                            const isPackageSelected = selectedPackage?.find(
                              (e) =>
                                e?.mandatory && option?._id === e?.option_id
                            );
                            if (!isPackageSelected) {
                              pkg.option_id = option?._id;
                              pkg.mandatory = true;
                      ...
                      >
                        <div
                          className={styles.SelecteddivHardCoded}
                          style={
                            selectedPackage?.find((e) => e?._id === pkg?._id)
                              ? { border: "2px solid #3A7AE4" }
                              : null
                          }
                        >
                          <span
        ...
        <Modal.Footer className={styles.FooterTime}>
          <div className={styles.FooterContaoner}>
            <div className={styles.wrappe} style={{ flexWrap: "wrap" }}>
              <div className={`${styles.TimeText} my-3`}>
                {durationPrice?.price === 0 && "There is no expert available"}
              </div>{" "}
              ...
              <div className={`${styles.Timetime} `}>
                {" "}
                {durationPrice?.duration !== undefined &&
                  `${durationPrice?.duration} Mins`}
              </div>
            </div>
            <div
              style={{
            ...
            >
              <ZouluButton
                title="Hinzufügen"
                className={`${styles.AddressButton1} `}
                onClick={() => onSubmit()}
              />
            </div>
          </div>
